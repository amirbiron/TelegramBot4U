...
1. **מול הלקוח הסופי (נושא המידע):** אין מסך הסכמה — אין לו איפה לחיות. הפתרון: (א) הצגה עצמית של הבוט — **חלק מהפרסונה ולא מנגנון נפרד**. ‏`build_system_prompt` פותח ב"אתה העוזר האישי שמנהל את התכתובות של {שם}", ובעל העסק מעצב את הניסוח דרך `custom_prompt`. שקלנו שורת גילוי ששורשרה מכנית לתשובה הראשונה, ונדחתה: היא כפילות של מה שהפרסונה כבר עושה, והיא מוציאה מידי בעל העסק החלטה שהיא שלו; (ב) כלל השקיפות בפרומפט (4.4) — **זה** האכיפה הדטרמיניסטית: הבוט לא מכחיש אוטומציה כשנשאל ישירות; (ג) בקשת מחיקה בשפה חופשית ("תמחקו את המידע שלי") מזוהה כ-intent וממופה ל-`delete_user_data`, עם אישור לבעלים.
...
| **4 — הקשחה ותאימות** | ‏edited/deleted מלא; חלון-סגור מלא; ‏retention; ‏privacy matrix; ‏backup; ‏Sentry | מוכן לפיילוט |
...
| 8 | הצגה עצמית — ‏tenant שינסח פרסונה שמכחישה אוטומציה | כלל השקיפות בפרומפט גובר על `custom_prompt`, ואינו ניתן לכיבוי |
